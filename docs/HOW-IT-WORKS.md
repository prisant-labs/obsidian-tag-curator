# How Tag Curator Works

Tag Curator changes how your tags *look* across Obsidian, never what they *are*. This page explains the idea in plain language first, then goes under the hood for engineers. The top half is for everyone; the "Under the hood" section and the second FAQ tier are for the technically curious.
...
Think of Tag Curator as a **view layer for your tags**. Like hiding rows in a spreadsheet without deleting the data, or putting on glasses that simplify what you see: the tags are all still there, in your notes and in Obsidian's own tag list. Tag Curator only changes what gets drawn on screen.
...
Vaults accumulate noisy tags: hex colors pasted from the web (`#FFAA00`), URL fragments (`#section-3`), one-off tags used in a single note. Tag Curator lets you **hide** noisy tags, **flag** them (highlight them in place instead of hiding), and pin the ones you always want to keep visible, across the places tags actually appear.

You do this from the **Curation Workspace**: the plugin's own dockable panel, where every tag in your vault appears in a table you can sort, search, and act on. Open it from the command palette with **Tag Curator: Open the panel**.
...
This is what the plugin does to each tag internally (not a click-by-click guide; for that, see the [README Quick start](../README.md#quick-start)). For every tag it encounters, Tag Curator:
...
Tag Curator can always tell you *why* a tag is hidden: every row in the Curation Workspace names the exact preset, rule, or override responsible. A tag is never hidden without a traceable reason. If anything ever looks wrong, **Tag Curator: Panic disable** removes every effect across every scope in a single action.

## Under the hood

*For engineers. The full reference, with the module map and sequence diagrams, lives in [ARCHITECTURE.md](ARCHITECTURE.md).*

**The prime directive: decorate, never mutate.** Every design choice follows from one rule - the plugin must not modify the vault's notes - so it works entirely at the rendered-DOM layer. To hide a tag, an observer adds a Tag-Curator-owned class (for example `.tag-curator-hidden`) to the rendered row, and CSS collapses it. Nothing leaves the document model and no note is rewritten.

**Four layers:**

- **UI surfaces** - the places tags render (the four scopes: tag pane, Notebook Navigator tree, Properties pills, Autocomplete popup), plus the plugin's own Curation Workspace. The Workspace is where you manage tags; it is not itself a curation scope.
...
**The honest dependency.** Because it decorates *rendered* rows, Tag Curator depends on the DOM structure and CSS class names of Obsidian's surfaces (and of Notebook Navigator), which are not a guaranteed public API. An Obsidian update could change that markup and require a selector fix in the plugin. Even then the failure mode is cosmetic - a tag might not hide correctly - and never touches your notes; disabling the plugin restores everything.

## FAQ

### Everyday

**Will Tag Curator change or delete my notes or tags?**
No, and this is built into the design rather than a policy it chooses to follow. Tag Curator contains no code that writes to your notes: it never calls any of Obsidian's note-editing APIs (the ones that create, modify, rename, or delete files, or rewrite frontmatter). The only files it writes are its own two settings files in its plugin folder. The single action that can change a note, renaming a tag across your vault, it hands to the separate Tag Wrangler plugin, and only when you ask for it. Your Markdown files and Obsidian's tag data are otherwise untouched.
...
**What happens if I disable or uninstall Tag Curator?**
...
Open the Curation Workspace (run **Tag Curator: Open the panel** from the command palette), find the tag's row, and pin it to **always-show** - it beats every rule. Or run **Tag Curator: Panic disable** to clear all effects at once.
...
No. Tag Curator makes zero network requests and has no telemetry. Nothing is fetched, and nothing is sent.
...
No. Those tools read Obsidian's own tag data, which Tag Curator never touches. They see your full, real tag set; only the visual UI changes.

**Is it free?**
Yes. Tag Curator is free and open source under the Apache 2.0 license.

**Where are my settings and rules stored?**
In your plugin folder, `.obsidian/plugins/tag-curator/`, as `data.json` (settings, rules, overrides) and `tags.json` (per-tag stats). Both are plain JSON, separate from your notes.
...
Per row, in order: always-show override, then always-hide override, then the highest-priority matching enabled rule, then the default (shown). That chain decides show-versus-hide only. "Flag" is a separate display treatment - the tag stays visible but highlighted in place rather than collapsed - used by preview mode (to show what a rule would hide) and available as a direct action on individual tags. Rule-priority assignment and tie-breaking are detailed in [ARCHITECTURE.md](ARCHITECTURE.md#visibility-resolution).

**Where is state stored, and why two files?**
`data.json` holds schema-versioned settings, rules, and overrides, with atomic writes (write-temp-then-rename). `tags.json` is a derived metadata sidecar (count, first and last seen, source), written on a debounce. They are split so frequent metadata updates never race settings writes. `tags.json` is derived, not a source of truth: if it is lost, **Tag Curator: Rescan vault tags** rebuilds it. Settings edited on another device (via Obsidian Sync rewriting `data.json`) are detected and reapplied.

**Does it couple to Notebook Navigator's code?**
No. It detects Notebook Navigator at runtime and decorates its rendered tag rows from the outside (runtime interop only), a silent no-op when Notebook Navigator is absent. There is no source coupling: Notebook Navigator is GPL-3.0, Tag Curator is Apache-2.0. (As with Obsidian's own surfaces, this is a dependency on rendered markup, not a public API - see "The honest dependency" above.)

**How do I see why a tag is hidden, or debug behavior?**
Every workspace row answers "why is this hidden?" by naming the exact preset, rule, or override. Advanced settings adds debug logging. Nothing is hidden without a traceable cause.
